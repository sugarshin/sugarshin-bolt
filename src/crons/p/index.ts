...
export const pCron = (app: App) => {
  const sayP = async () => {
    ...
    const selectCommon = () => {
      ...
      const commonPromises = splited.slice((commonHeader.index + 1), headers[1].index).filter(row => orderedListRegex.test(row));
      return commonPromises[random(0, commonPromises.length - 1)];
    };

    const promises0 = splited.slice((headers[1].index + 1), headers[2].index).filter(row => orderedListRegex.test(row));
    const promises1 = splited.slice((headers[2].index + 1), splited.length).filter(row => orderedListRegex.test(row));
...
${commonHeader && selectedCommon ? `✅ ${commonHeader.text.replace(/^##\s+/, '')}： ${selectedCommon}` : ''}
✅ ${name}： ${selectedPerson}
