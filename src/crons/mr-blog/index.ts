...
export const mrBlogCron = () => {
  const executeMonthlyReport = async () => {
    try {
      const res = await got.post(`https://circleci.com/api/v1/project/sugarshin/build.blog.sugarshin.net/tree/monthly-report?circle-token=${TOKEN}`);
