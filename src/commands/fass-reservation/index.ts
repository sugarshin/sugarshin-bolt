...
const getWaitingResultPageUrl = (salonId: string) =>
...
const getWaitingOrder = (html: string) =>
  parseWaitingOrderText(cheerio.load(html)(selectors.waitingOrder).text());
...
export const fassReservation = (app: App): void => {
  app.command('/fass', async ({ body, ack, say }) => {
    ...
    if ((m = body.text.match(/^\s*([a-z0-9]+)\s+w(?:aiting)?(?:\s+(\d+))?\s*$/i))) {
      ...
      fetchHtm(waitingResultPageUrl).then((html: string) => {
        ...
        const fetchHtmPolling = createPoll(fetchHtm, (html: string) => {
        ...
        fetchHtmPolling(waitingResultPageUrl).catch(err =>
          say(`エラーが発生しました ${err.toString()}`),
        );
      });
